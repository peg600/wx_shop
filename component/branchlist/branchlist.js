...
Component({
  ...
  properties: {
    ...
    mode: {
      type: Number,
      value: 0, //0:指定branchs参数，1:自动获取 2:LBS
    },
    limit:
      {
        type: Number,
  ...
  methods: {
    _toDetailListPage: function (e) {
      ...
      wx.navigateTo({
        url: "/pages/personalbranch/personalbranch?user_id=" + user_id,
  ...
  attached: function () {
    ...
    switch (this.data.mode) {
      case 0:
        {
          var ids = this.data.branches;
          wx.request({
            url: baseUrl + '/?c=branch&a=getBranchInfo&ids=' + ids,
            method: "POST",
            dataType: 'txt',
            success: res => {
              function Trim(str) {
      ...
      case 1:
        {
          ...
          wx.request({
            url: baseUrl + '/?c=branch&a=getBranches',
      ...
      case 2:
        {
          wx.getLocation({
            success: function (res) {
              ...
              wx.request({
                url: baseUrl + '/?c=branch&a=getNearBranches',
                header: { 'content-type': 'application/x-www-form-urlencoded' },
                method: "POST",
                dataType: 'txt',
                data: {
